...
# CSS ΓΙΑ ΤΟ ΤΕΛΙΚΟ LOOK (v16.0 - Locked Light Map & Bio Position)
st.markdown("""
    <style>
    ...
    /* Force Light Map Theme */
    [data-testid="stMap"] { filter: invert(0) !important; border-radius: 15px; }
    </style>
...
# 3. Εικόνα Συσκευασίας (Μόνη της πλέον)
...
# 6. Drone Footage & Βιολογική Πιστοποίηση (Νέα Θέση)
st.markdown('<div class="section-header">🚁 Drone Spraying (Real Footage)</div>', unsafe_allow_html=True)
st.video(p['video'])

# Κάρτα με Πτήσεις και Σήμα Bio αν ισχύει
with st.container():
    content = ""
    if p['show_bio'] and os.path.exists(p['bio_img']):
        # Δημιουργία HTML για το σήμα και το κείμενο
        st.markdown(f"""
        <div class="info-card" style="border-left: 5px solid #2e7d32;">
            <div class="bio-container-drone">
                <img src="https://raw.githubusercontent.com/{st.secrets.get('GH_USER', 'your-user')}/{st.secrets.get('GH_REPO', 'your-repo')}/main/bio.png" width="50" style="vertical-align:middle;">
...
qr_url = "https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=https://hasia-beans.streamlit.app"
col_qr, col_txt = st.columns([1, 2])
with col_qr:
    st.image(qr_url, caption="Batch QR", width=100)
with col_txt:
    st.markdown("<p style='font-size:12px; margin-top:10px; color:#333;'>Σκανάρετε για επαλήθευση αυθεντικότητας παρτίδας.</p>", unsafe_allow_html=True)

# 12. Αγρόκτημα Button
if st.button("⭐ Κλείστε Ξενάγηση στο Αγρόκτημα"):
    st.balloons()

st.markdown("<p style='text-align:center; font-size:10px; color:#999; margin-top:15px;'>ΟΣΠΡΙΑ ΧΑΣΙΩΝ DPP v16.0</p>", unsafe_allow_html=True)
